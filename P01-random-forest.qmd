...
:::{.callout-tip}
The estimated value of $Y$ (the dependent variable) for $X = x$ is the average of $Y$ from all the observations that belong to the same tree as $X = x$.
:::
...
This means `value_seq[70]` (`r value_seq[70]`) is a better threshold than `value_seq[50]` (`r value_seq[50]`).
