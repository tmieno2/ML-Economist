...
where $L()$ is the user-specified loss-function that is differentiable and $\Omega(w_j)$ is the regularization term. Instead of @eq-obj, XGB uses the second order Taylor expansion of $L()$ about $w$^[This helps for some of the commonly used loss functions]. 
