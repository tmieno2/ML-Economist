...
:::{.callout-note}
If the value of $\gamma$ is lower (say, 1), then we would have adopted the first split. 

```{r}
get_info(data_R_1, 5, lambda, 1)$gain # first split
get_info(data_R_1, 7, lambda, 1)$gain # second split
```
