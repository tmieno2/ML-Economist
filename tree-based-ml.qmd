...
A variant of gradient boosting that has been extremely popular for its performance is extreme gradient boosting. While it follows the gradient boosting steps described above mostly, it has its own way of building a tree. In particular, they take more measures to avoid over-fitting in building each tree. 
...
$$
L(y_i, p_i) + \gamma T + \frac{1}{2}\lambda ||||
$$

$$
\begin{aligned} 
\frac{\partial L(y_i, p_i)}{\partial p_i}  = -(y_i - p_i)\\
\frac{\partial^2 L(y_i, p_i)}{\partial p_i^2} = p_i \\
\end{aligned} 
$$
