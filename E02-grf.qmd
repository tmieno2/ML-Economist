...
A GRF with $\Psi_{\theta, \nu}(Y_i, X_i, T_i) = (Y_i - E[Y|X])- \theta(X)(T_i - E[T|X])$ is causal forest as an R-learner (take a look at @sec-r-learner to see the reasoning behind this score function). In practice $E[Y|X]$ and $E[T|X]$ are first estimated using appropriate machine learning methods (e.g., lasso, random forest, gradient boosted forest) in a cross-fitting manner and then the estimation of $Y_i - E[Y|X]$ and $T_i - E[T|X]$ are constructed. Let's denote them by $\hat{\tilde{Y}}_i$ and $\hat{\tilde{T}}_i$. Then the empirical score function is written as
...
Here, we look some of the hyper-parameters that are <span style='color:blue'>common </span>across the methods under the GRF framework. Since the various GRF methods vary only on how the pseudo outcome is defined, it is natural that they share many hyper-parameters. There are of course method-specific hyper-parameters and we will look at such parameters when we discuss them individually.

Further, since GRF basically uses the RF algorithm, it is natural that some of the GRF hyper-parameters are the same as the ones for RF (by `ranger()`).
...
GRF uses sampling without replacement when determining the samples for building each tree. RF implemented by `ranger()`` uses 

`sample.fraction`, which we saw earlier. 
